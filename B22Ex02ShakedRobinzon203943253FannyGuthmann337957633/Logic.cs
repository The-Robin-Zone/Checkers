...
namespace B22Ex02ShakedRobinzon203943253FannyGuthmann337957633
{
    public class Logic
    {
            ...
            for (int i = 1; i < gameBoard.BoardSize - 1; i++)
            {
                for (int j = 1; j < gameBoard.BoardSize - 1; j++)
                {
                    if (tileOccupiedByColor(gameBoard, i, j, playerColor))
                    {
                        ...
                        if (tileOccupiedByColor(gameBoard, xPoint, j + 1, opponentColor) &&
                            !coinIsAtBorderOfBoard(gameBoard, xPoint, j + 1) &&
                             IsTileFree(gameBoard, xEndJump, j + 2))
                        {
        ...
        // if capacity is equal to zero it means we dont have any element in the array
        //Console.WriteLine(allMovePossible.Capacity);
